...
def db_get_pending_reminders() -> list[dict]:
    """Возвращает задачи которым пора напомнить, с учётом часового пояса каждого пользователя."""
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    # Берём все невыполненные задачи с remind_at
    rows = conn.execute(
        "SELECT t.*, s.timezone FROM tasks t "
        "LEFT JOIN user_settings s ON t.user_id = s.user_id "
        "WHERE t.remind_at IS NOT NULL AND t.reminded = 0 AND t.done = 0"
    ).fetchall()
    conn.close()
    ...
    for row in rows:
        d = dict(row)
        tz_str = d.get("timezone") or "Europe/Moscow"
        now_str = now_local(tz_str).strftime("%Y-%m-%d %H:%M")
        if d["remind_at"] <= now_str:
            result.append(d)
    return result
...
async def _process_task(update: Update, text: str):
    ...
    if due_time:
        try:
            due_dt = datetime.strptime(due_time, "%Y-%m-%d %H:%M")
            if remind_min > 0:
                # Напомнить за N минут до срока
                remind_at = (due_dt - timedelta(minutes=remind_min)).strftime("%Y-%m-%d %H:%M")
            else:
                # remind_minutes_before=0 значит напомнить ровно в момент срока
                remind_at = due_time
        except ValueError:
            pass
...
async def check_reminders(app: Application):
    # Отладка — логируем все задачи с remind_at
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        all_tasks = conn.execute(
            "SELECT id, text, remind_at, reminded, done FROM tasks WHERE remind_at IS NOT NULL AND done = 0"
        ).fetchall()
        conn.close()
        for t in all_tasks:
            tz_str = "Europe/Moscow"
            now_str = now_local(tz_str).strftime("%Y-%m-%d %H:%M")
            logger.info(f"Задача #{t['id']}: remind_at={t['remind_at']} | now={now_str} | reminded={t['reminded']} | подходит={t['remind_at'] <= now_str}")
    except Exception as e:
        logger.error(f"Ошибка отладки: {e}")

    for task in db_get_pending_reminders():
        try:
            tz_str = task.get("timezone") or "Europe/Moscow"
            ...
            if task["due_time"]:
                try:
                    ...
                    if mins_left <= 0:
                        ...
                        h = mins_left // 60
                        m = mins_left % 60
                        time_left = f"через {h} ч {m} мин" if m else f"через {h} ч"
                    else:
            ...
            if mins_left is not None and 4 <= mins_left <= 6:
                reminder_text = (
                    f"⏰ *Скоро!* Через 5 минут:\n"
                    f"━━━━━━━━━━━━━━━━\n"
                    f"📋 *{task['text']}*\n"
                    f"━━━━━━━━━━━━━━━━\n"
                    f"📅 *Срок:* {due_fmt}"
                ...
                reminder_text = (
                    ...
                    f"━━━━━━━━━━━━━━━━\n"
                    f"📅 *Срок:* {due_fmt}\n"
                    + (f"⏳ *Осталось:* {time_left}" if time_left else "")
                )
